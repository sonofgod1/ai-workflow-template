...
3. **Nunca hagas commits ni pushes.** Yo hago los commits. Tú me dices qué cambiaste y por qué. **Única excepción:** si este proyecto creó `.workflow/delivery.conf` con `MODO_ENTREGA=pr`, `/ship` puede commitear y pushear **la branch de trabajo** y abrir el PR. Nunca la base, y **el merge nunca** (ver "Modos de entrega").
...
el PR**. Se activa creando `.workflow/delivery.conf`:
...
3. /build docs/plans/[archivo]        (implementa, con su test de regresión)
