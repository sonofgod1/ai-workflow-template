...
Esto declara la fase y activa su política de escritura: en `/implement` los hooks
...
## VERIFICAR ANTES DE REPORTAR — obligatorio

No existe "implementación completada" sin evidencia. Antes de escribir el reporte,
corre el contrato de verificación del proyecto:

```bash
bash .workflow/verify.sh
```

Esto corre lint, type-check y tests con los comandos que el proyecto declaró
(`.workflow/verify.conf`) o los que se detecten por stack, y deja la evidencia en
`.workflow/.last-verify.json`.

**Las tres salidas y qué hacer con cada una:**

- `ok` → sigue al reporte y pega el resumen.
- `falla` → **no reportes todavía**. Corrige y vuelve a correr. Si no puedes
  corregirlo (el fallo es preexistente y ajeno a tu cambio), repórtalo como
  hallazgo y dilo explícitamente en el reporte: nunca lo escondas.
- `parcial` → hay pasos saltados por falta de herramienta. **Esto no es verde.**
  Dilo en el reporte, nombra qué quedó sin verificar, y di qué haría falta para
  cerrarlo.

Correr la verificación no reemplaza la prueba manual: la verificación demuestra
que no rompiste lo que ya estaba cubierto, la prueba manual demuestra que lo
nuevo hace lo que se pidió. Hacen falta las dos.
...
### Verificación — pegar la salida real, no describirla
[Bloque literal del resumen de `bash .workflow/verify.sh`:
...
6. git commit -m "fix(ID): descripción"          ← código
